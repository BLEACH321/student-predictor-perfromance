import { UserCircle, Settings, LogOut, Bell, Search, Menu } from 'lucide-react';
...
export function AdminHeader({ onLogout, onToggleSidebar }: AdminHeaderProps) {
  return (
    <header className="bg-card border-b border-border/50 px-4 md:px-8 py-4 sticky top-0 z-30 shadow-premium-sm bg-white/80 backdrop-blur-md">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        <div className="flex items-center gap-4 md:gap-8">
          <button 
            onClick={onToggleSidebar}
            className="lg:hidden p-2 rounded-xl bg-muted/50 text-foreground hover:bg-muted transition-all"
          >
            <Menu size={24} />
          </button>

          <div className="hidden md:flex items-center gap-2 bg-muted/50 border border-border/50 rounded-2xl px-4 py-2 w-72 lg:w-96 group focus-within:bg-card focus-within:ring-4 focus-within:ring-primary/5 transition-all">
          ...
          <div className="flex items-center gap-2 md:gap-3">
            <div className="flex items-center gap-1 bg-muted/30 p-1 rounded-2xl border border-border/50">
              <button className="p-2 md:p-2.5 rounded-xl hover:bg-card hover:text-primary transition-all text-muted-foreground relative group">
                <Bell size={20} />
              ...
              <button className="hidden sm:flex p-2 md:p-2.5 rounded-xl hover:bg-card hover:text-primary transition-all text-muted-foreground group relative">
                <Settings size={20} />
            ...
            <button
              onClick={onLogout}
              className="p-2 md:p-2.5 rounded-xl bg-destructive/10 text-destructive hover:bg-destructive hover:text-white transition-all group relative border border-destructive/10"
            >
              <LogOut size={20} />
